...
# REVISAR DOCUMENTACIÓN POR SI SE NECESITA FILTROS
...
while read -r elemento; do
    ...
    url_repo_github=$(curl \
        -H "Authorization: token $TOKEN_GITHUB" \
        -H "Accept: application/vnd.github.v3+json" \
        "https://api.github.com/repos/${OWNER}/${repositorio}" |
        jq -r '.clone_url')

    # 1. Clonar el repositorio de GitLab como espejo
    # Se utiliza un nombre fijo para evitar depender del "path"
    # de GitLab.
    echo "CLONANDO REPOSITORIO DE GITLAB..."

    git clone --mirror "$url_repo_gitlab" repositorio.git

    # Validar que el clon se haya creado correctamente
    ...
    # 2. Entrar en el repositorio clonado
    cd repositorio.git
